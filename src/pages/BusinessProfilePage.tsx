    ...
    <main className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="text-center max-w-md">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link
            to="/mercato"
            className="px-5 py-2.5 bg-accent text-charcoal text-sm font-semibold rounded-xl hover:bg-[#c4963e] transition-colors"
          >
            Browse Mercato
          </Link>
          <Link
            to="/"
            className="px-5 py-2.5 border border-border text-charcoal text-sm font-medium rounded-xl hover:border-accent hover:text-accent transition-colors"
          >
            Back to Village
          </Link>
        </div>
...
function OfferCard({ offer, businessName }: { offer: Business['offers'][number]; businessName: string }) {
  return (
    <article
      className="bg-surface rounded-2xl p-5 border border-border flex flex-col gap-3"
...
export function BusinessProfilePage() {
  ...
  // Related businesses: industry (+3), location (+2), shared topic (+1) — exclude self
    ...
    <main className="min-h-screen bg-background">
      ...
      <section aria-labelledby="business-title">
        ...
        <div className="relative h-56 sm:h-72 md:h-80 overflow-hidden bg-charcoal">
          ...
          />
          {business.featured && (
            <span className="absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-medium bg-accent text-charcoal">
              Featured
            </span>
          )}
        </div>
        ...
        <div className="bg-surface border-b border-border pb-10">
          <InnerContainer>
            <div className="flex items-end gap-5 -mt-10 relative z-10 mb-6">
              ...
              {/* Industry badge — next to logo at bottom of cover */}
              <div className="pb-1">
            ...
            <h1
              id="business-title"
              className="font-heading text-3xl sm:text-4xl md:text-5xl font-bold text-charcoal leading-tight mb-2"
            >
              {business.name}
            </h1>

            {/* Tagline */}
            <p className="font-heading text-lg text-muted italic mb-4">
              {business.tagline}
            </p>

            {/* Description */}
            <p className="font-body text-base text-muted leading-relaxed mb-6 max-w-2xl">
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm font-body text-muted mb-5">
              ...
              <time dateTime={business.createdAt} className="text-muted">
        ...
        <InnerContainer>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-14">
            ...
            <div className="lg:col-span-2 flex flex-col gap-14">
              ...
              {business.offers.length > 0 && (
                <section aria-labelledby="business-offers-heading">
                  ...
                  <p className="font-body text-sm text-muted mb-6">
                    Services and products from this business, available directly.
                  </p>
                  <ul
                    className="grid grid-cols-1 sm:grid-cols-2 gap-4"
                    role="list"
                    aria-label={`Offers from ${business.name}`}
                  >
                    {business.offers.map(offer => (
                      <li key={offer.id}>
                        <OfferCard offer={offer} businessName={business.name} />
                      </li>
                    ))}
                  </ul>
                </section>
              ...
              {/* Stories from this business */}
              <section aria-labelledby="business-stories-heading">
                <h2
                  id="business-stories-heading"
                  className="font-heading text-2xl font-semibold text-charcoal mb-2"
                >
                  Stories from {business.name}
                </h2>
                <p className="font-body text-sm text-muted mb-6">
                  Published founder stories connected to this business.
                </p>
                <StoryGrid
              ...
              {/* Ideas connected to this business */}
              <section aria-labelledby="business-ideas-heading">
                <h2
                  id="business-ideas-heading"
                  className="font-heading text-2xl font-semibold text-charcoal mb-2"
                >
                  Ideas connected to {business.name}
                </h2>
                <p className="font-body text-sm text-muted mb-6">
                  Knowledge extracted from this business's stories.
                </p>
                <IdeaGrid
                  filter={{ businessId: business.id }}
                  columns={2}
              ...
              {founder && (
                <section aria-labelledby="business-founder-heading">
                  <h2
                    id="business-founder-heading"
                    className="font-heading text-2xl font-semibold text-charcoal mb-2"
                  >
                    The Founder
                  </h2>
                  <p className="font-body text-sm text-muted mb-6">
                    The person behind {business.name}.
                  </p>
            ...
            <aside className="lg:col-span-1" aria-label="Business details">
              <div className="sticky top-24 flex flex-col gap-6">
                ...
                {/* Primary offer CTA — highlighted */}
                {business.offers[0] && (
                  <div className="bg-accent/10 rounded-2xl p-5 border border-accent/30">
                    <p className="font-body text-xs font-semibold text-accent uppercase tracking-wide mb-2">
                      Top Offer
                    </p>
                ...
                >
                  ...
                  <dl className="space-y-4 font-body text-sm">
                    ...
                    <div>
                      ...
                      <dd>
                        <time dateTime={business.createdAt} className="text-charcoal">
